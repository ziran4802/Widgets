...
### 2026-09-12 Codex 额度 WorkerW 点击通道修复

- 额度组件此前未进入 WorkerW 的 interactive/native mouse router，导致桌面附着时“确认额度/重新确认”按钮无法收到点击；现与每日待办共用限定的桌面交互通道，其他组件仍保持点击穿透。
- 新增 WorkerW 路由回归，确认额度组件使用 interactive 输入模式并进入桌面路由目标列表；`npm.cmd test`：122/122 通过。
- `npm.cmd run smoke:codex-quota`、`npm.cmd run package:portable` 和 `npm.cmd run smoke:portable` 均通过。上述额度 smoke 使用临时 floating 宿主，不替代真实 WorkerW 可见桌面验证。
- `npm.cmd run smoke:todo-desktop` 本轮未进入业务断言，因当前 Electron 环境 GPU 进程不可用（`GPU process isn't usable`）；真实 WorkerW 下额度按钮仍需用户重启成品后人工确认。
...
### 2026-09-10 鼠标无法移入待办的回归修复

- 用户确认上一版交互态下鼠标无法移入待办。本次在正在运行的组件边缘用系统鼠标移动复现：目标位置已在组件内，实际光标仍停在组件外。
- 原因是鼠标转发钩子同时吞掉了 `WM_MOUSEMOVE`。现保留控件的移动通知，但让系统继续处理移动事件；点击和滚轮仍按原范围处理。
- 旧版桌面 smoke 用 `SetCursorPos` 直接设置光标位置，未覆盖此故障。现在改为发送系统移动并回读真实位置，无法移入时立即失败；测试组件位于主屏右下方，以避开默认左侧的用户待办。
- 本次 `npm.cmd test`：116/116 通过；更新的 `npm.cmd run smoke:todo-desktop -- --minimize-obstruction` 通过，包含 `cursorMoved`、输入、新增、勾选、锁定/解锁及正常退出。开发中的一次勾选检查失败，补充点击目标诊断后完整运行通过；真实用户组件仍需重启确认。

### 2026-09-10 空任务按钮聚焦修复

- 用户确认鼠标已可点击，但点击“添加任务”后无法输入。代码检查发现空表单提交直接返回，焦点停留在按钮；现为空提交时聚焦左侧输入框，保留 WorkerW 模式。
- 本次 `npm.cmd test`：116/116 通过。桌面隔离 smoke 改为先点击空表单的添加按钮，验证 DOM 输入焦点、系统输入、新增和勾选，全流程通过。
- 另一次隔离诊断中，直接点击输入框后普通虚拟键 A 输入成功，前台线程焦点与目标窗口一致；这不代表中文输入法或真实用户组件已验证。用户本次问题是否完全解决仍待确认。
