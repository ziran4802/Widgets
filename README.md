...
- Codex 额度：可选的本地 Codex app-server 读取，优先显示上次成功确认的 5 小时/周窗口、已用比例和重置信息；点击“确认额度/重新确认”才读取最新数据，WorkerW 桌面附着时仍保留该按钮的点击通道，不显示费用或凭据。
- 管理器：组件目录分页、紧凑实例列表分页、布局编辑、外观设置和自启动入口；目录与实例页数按实际数量和窗口容量计算。
- 托盘常驻：关闭管理器后可保留组件运行，并从托盘打开、隐藏/恢复组件或退出 Widget。
- 主屏桌面组件：正式运行尝试附着 Windows WorkerW 桌面层；编辑布局时临时使用普通窗口以便拖动，完成或取消后重新附着。
...
- Codex 额度是可选组件：本机没有可用的 `codex app-server` 时会优先保留上次结果并提示本次确认失败；没有历史结果时显示不可用状态。快照保存在 userData 下的独立本地文件，不写入组件配置。项目不把“使用本机进程”描述为全链路离线。
...
- Codex 额度不自动首刷或定时刷新；启动时显示最近一次成功快照，用户点击按钮后才发起本地确认。缓存损坏或不存在时会安全回退到首次确认状态。
- WorkerW、GPU/网络性能计数器、托盘和当前用户自启动依赖 Windows 实际环境；开发版和自动化隔离配置不会修改真实自启动项。
- 每日待办在真实桌面上的锁定态点击穿透、托盘解锁、新增/勾选保存、跨午夜清空和长文本边界仍需要人工抽查；Codex 额度在真实 WorkerW 桌面层的按钮点击和登录读取、中文便签输入、编辑拖动、Explorer 重启、注销登录恢复以及 165% DPI 可读性也需要在普通桌面会话中人工抽查。
