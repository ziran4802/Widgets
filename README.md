...
- 每日待办：本地记录当天任务，支持新增、勾选完成、进度显示；启动进入新日期时清空上一日任务，不提供历史记录、提醒或同步。
- Codex 额度：可选的本地 Codex app-server 读取，显示 5 小时/周窗口、已用比例和重置信息，不显示费用或凭据。
- 管理器：组件目录、实例状态、布局编辑、外观设置和自启动入口。
...
- 每日待办只保存本机当前日期的任务，不包含历史归档、提醒、重复任务或跨设备同步。
- WorkerW、GPU/网络性能计数器、托盘和当前用户自启动依赖 Windows 实际环境；开发版和自动化隔离配置不会修改真实自启动项。
- 每日待办在真实桌面上的锁定态勾选、新增后保存、跨午夜清空和长文本边界仍需要人工抽查；真实 Codex 登录读取、中文便签输入、锁定态点击穿透、编辑拖动、Explorer 重启、注销登录恢复以及 165% DPI 可读性也需要在普通桌面会话中人工抽查。
